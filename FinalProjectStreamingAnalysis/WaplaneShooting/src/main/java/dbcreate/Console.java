...
public class Console extends WindowAdapter implements WindowListener, ActionListener, Runnable
{
	...
	public Console()
	{
		...
		pnlEast = new JPanel();
		pnlEast.setPreferredSize(new Dimension(200, 200));
		...
		pnlEast.setLayout(new GridLayout(10,0));
		pointX = new JTextField(10);
		pointY = new JTextField(10);
		pnlEast.add(pointX);
		pnlEast.add(pointY);
		btnInitialize = new JButton("Generate");
		btnShoot = new JButton("Point");
		btnDropTable = new JButton("Drop");
		btnClear = new JButton("Clear");
		pnlEast.add(btnInitialize);
		pnlEast.add(btnShoot);
		pnlEast.add(btnDropTable);
	...
	public synchronized void actionPerformed(ActionEvent evt)
	{
		...
		if (evt.getActionCommand().equals("Generate")) {
			if(dbo == null) 
				dbo = new  InitializeLocations();			
		}
		if (evt.getActionCommand().equals("Point")) {
				int x = Integer.parseInt(pointX.getText());
				int y = Integer.parseInt(pointY.getText());
				dbo.executeQuery(InitializeLocations.SELECT_EXCEPT_POINT, new Point(x, y));
				}
		if (evt.getActionCommand().equals("Drop")) {
			if(dbo != null) 
			dbo.executeQuery(InitializeLocations.DROP_TABLE);
		}
		if (evt.getActionCommand().equals("Clear")) {
			if(dbo != null) 
			dbo.executeQuery("");
		}
		} catch(Exception msg) { System.out.println(msg.getMessage()); } 
	}
	...
	public static void main(String[] arg)
	{
		...
		System.out.println("My output 08:13");
	}			
}